...
Here is a quick graph to show you the count of 'heads' in these two trials:

```{r graph two trials binomial outcome, echo=F,fig.dim=c(6,3),message=F}
x.x1<- tibble(x,x1)
x.x1

x.x1 %>% gather(trial,value) %>% group_by(trial)  %>% 
  ggplot(aes(x=trial,fill=trial,color=trial))+ geom_histogram(stat = 'count')#+facet_grid(~trial)
#+
