...
\[H_0: \hspace{5pt} X \hspace{5pt}= \hspace{5pt}Y \]

2. The Experimental Hypothesis (also known as the Alternative Hypo. )

\[H_a: \hspace{5pt} X \hspace{5pt} != \hspace{5pt}Y \]
...
\[H_0: X \hspace{5pt} = \hspace{5pt} Y \]

or it can also be said like this:

\[H_0: X \hspace{5pt} - \hspace{5pt} Y \hspace{5pt} = \hspace{5pt} 0\]
...
If we flip a coin 10 times, or if we have 10 coins and flip them once, what is the chance that we'll have 10 heads or 10 tails?

Here is a quick simulation of 10 coin flips (assuming a fair coin of .5)
...
If we count up the number of heads and divide by the number of flips, we get `r sum(x)/10`.  
...
##Plotting of many simulated coin tosses

Let's say I build a random coin flipper and flip it many times.
Here is some output:
...
So, this is just the top 6 simulations out of a `r sims` simulations.
Each row has two simulations of flipping a coin 10 times.

It's like our soda experiment, but we are doing it many times.
And it's random.
So it is a way for us to think about what we should expect to get if our experiment were just random chance.  

It's like asking this question:  Our experiment found a 8 vs 3 fizzes.  How often would we find that difference if this was all just random? 


##Counting the number of 8 vs 3

How many times did this random simulation match our No Tap condition of 8 and Tap condition of 3? 
...
flip.df[flip.df$NoTap>=8 & flip.df$Tap<=3,]


nrow(flip.df[flip.df$NoTap>=8,])/10000
sum(dbinom(10:8,10,prob=.5))
nrow(flip.df[flip.df$NoTap<=3,])/10000
sum(dbinom(3:0,10,prob=.5))
```

Not too often.
If I sum the data from behind the scenes, I get just `r sum(count$count)` out of `r sims`.  
